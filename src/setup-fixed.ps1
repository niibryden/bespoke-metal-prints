...
if (Test-Path $functionsPath) {
    Write-Host "   [OK] Folder already exists: $functionsPath" -ForegroundColor Green
} else {
    New-Item -ItemType Directory -Path $functionsPath -Force | Out-Null
    Write-Host "   [OK] Created folder: $functionsPath" -ForegroundColor Green
}
Write-Host ""

# Step 2: Download files from Figma Make
Write-Host "Step 2: Checking Edge Function files..." -ForegroundColor Yellow
Write-Host ""
Write-Host "IMPORTANT: You need to download the Edge Function files from Figma Make" -ForegroundColor Red
Write-Host ""
...
if (-not $allFilesExist) {
    Write-Host "WARNING: Some files are missing. Please copy them from Figma Make before continuing." -ForegroundColor Red
    Write-Host ""
...
Write-Host "[SUCCESS] All Edge Function files are in place!" -ForegroundColor Green
Write-Host ""
...
if (Test-Path $supabaseConfigPath) {
    Write-Host "   [OK] Supabase already initialized" -ForegroundColor Green
} else {
    Write-Host "   Initializing Supabase..." -ForegroundColor Cyan
    try {
        supabase init
        Write-Host "   [OK] Supabase initialized successfully" -ForegroundColor Green
    } catch {
        Write-Host "   [ERROR] Error initializing Supabase: $_" -ForegroundColor Red
...
try {
    ...
    Write-Host "   WARNING: Not logged in. Opening login..." -ForegroundColor Yellow
    Write-Host ""
...
try {
    ...
    Write-Host "   [OK] Project linked: $projectRef" -ForegroundColor Green
} catch {
    Write-Host "   WARNING: Project may already be linked (this is okay)" -ForegroundColor Yellow
}
Write-Host ""
...
try {
    ...
    if ($response.success -eq $true) {
        Write-Host "   [OK] Edge Function is responding!" -ForegroundColor Green
        Write-Host "   Response: $($response | ConvertTo-Json -Compress)" -ForegroundColor Cyan
    } else {
        Write-Host "   WARNING: Edge Function responded but returned unexpected data" -ForegroundColor Yellow
        Write-Host "   Response: $($response | ConvertTo-Json)" -ForegroundColor Yellow
    }
} catch {
    Write-Host "   [ERROR] Edge Function test failed: $_" -ForegroundColor Red
